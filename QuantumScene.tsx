...
// FIX: Refine the global JSX IntrinsicElements augmentation for React Three Fiber.
// By extending ThreeElements from @react-three/fiber and explicitly adding common Three.js
// intrinsic elements, we ensure they are correctly recognized within the JSX namespace
// regardless of the specific environment's TypeScript configuration or React version.
declare global {
  namespace JSX {
...
// FIX: Also augment the React module's JSX namespace to handle different build tools
// that might look for IntrinsicElements within the React namespace specifically.
...
const QuantumParticle = ({ position, color, scale = 1 }: { position: [number, number, number]; color: string; scale?: number }) => {
  // FIX: Use imported `THREE.Mesh` type for the ref.
...
const MacroscopicWave = () => {
  ...
  return (
    <Torus ref={ref} args={[3, 0.1, 16, 100]} rotation={[Math.PI / 2, 0, 0]}>
      {/* FIX: meshStandardMaterial is now recognized via the explicit JSX augmentation */}
  ...
  return (
    <div className="absolute inset-0 z-0 opacity-60 pointer-events-none">
      <Canvas camera={{ position: [0, 0, 6], fov: 45 }}>
        {/* FIX: ambientLight and pointLight are now recognized as intrinsic elements */}
  ...
  return (
    <div className="w-full h-full absolute inset-0">
      <Canvas camera={{ position: [0, 0, 4.5], fov: 45 }}>
        {/* FIX: ambientLight, spotLight, and pointLight are now recognized as intrinsic elements */}
        ...
        <Float rotationIntensity={0.4} floatIntensity={0.2} speed={1}>
          {/* FIX: group is now recognized via the explicit JSX augmentation */}
