...
def main():
    ...
    if page == "1. 總覽與查詢": page_search()
    elif page == "2. 入庫作業": page_operation("入庫")
    elif page == "3. 出庫作業": page_operation("出庫")
    elif page == "4. 新增項目": page_add_single()
    elif page == "5. 編輯表格": page_edit_table()
    elif page == "6. 批次匯入 (CSV)": page_import_csv()
    elif page == "7. 批次匯入 (圖片)": page_import_images()
    elif page == "8. 異動紀錄": page_reports()
    elif page == "9. 資料庫重置": page_reset_db()
...
def page_search():
    ...
    st.dataframe(result, column_config={"ImageFile": st.column_config.ImageColumn("圖片"), "Stock": st.column_config.NumberColumn("庫存")}, use_container_width=True)
    ...
    if up_csv:
        try:
            ...
            if df_im is not None:
                ...
                st.write("預覽資料："); st.dataframe(df_im.head())
            ...
            else: st.error("無法讀取 CSV，請檢查編碼或格式。")
...
def page_import_images():
    ...
    if not all_skus:
        st.warning("⚠️ 資料庫目前是空的，無法進行圖片對應。請先去【批次匯入 (CSV)】上傳商品資料。")
    else:
        ...
        if imgs and st.button("開始批次上傳"):
            ...
            st.success(f"處理完成！成功: {succ}, 跳過(找不到SKU): {fail}"); time.sleep(3); st.rerun()
